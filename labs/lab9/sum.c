...
static int sum_vectorized_unrolled(int n, int *a)
{
	...
	__m128i ai, sum = _mm_setzero_si128();
	for(int i = 0; i < n / 16 * 16; i+=16){
		sum = _mm_add_epi32(sum, _mm_loadu_si128(a+i));
		sum = _mm_add_epi32(sum, _mm_loadu_si128(a+i+4));
		sum = _mm_add_epi32(sum, _mm_loadu_si128(a+i+8));
		sum = _mm_add_epi32(sum, _mm_loadu_si128(a+i+12));
	}
	int s[4];
	_mm_storeu_si128(s, sum);
